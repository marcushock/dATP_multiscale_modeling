...
k2_minus        =  0.000000159; // k2_plus_baseline / exp(g_M1 - g_M2); //0.5 / exp(g_Mc - g_Md);    //using vals from optimization_0227 (k1_plus = 0.615440)

//k3_plus         = (B/pow(k_xb,.5))*(1-tanh(C*pow(k_xb,.5)*(x_xb-x_b0)))+D;        //X_k3_plus_PSO[i];
k3_minus        = 0.0278; // k3_plus_baseline / exp(g_M2 - g_M3) ;//0.3 / exp(g_Mc - g_Md);  //
